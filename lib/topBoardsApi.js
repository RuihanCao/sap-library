...
const { CURRENT_GAME_VERSION } = require("./config");
...
async function getCurrentVersion(pool) {
  if (CURRENT_GAME_VERSION) return CURRENT_GAME_VERSION;
  ...
  const versions = rows
    .map((row) => normalizeVersionValue(row.game_version))
    .filter(Boolean)
...
async function resolveVersion(pool, value) {
  const normalized = normalizeVersionValue(value || "current").toLowerCase();
  if (!normalized || normalized === "all") return null;
  if (normalized !== "current") return normalizeVersionValue(value);
  return await getCurrentVersion(pool);
}
